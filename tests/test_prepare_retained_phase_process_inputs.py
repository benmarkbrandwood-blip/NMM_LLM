"""Tests for successor-owned retained phase-process input snapshots."""
...
import tools.prepare_retained_phase_process_inputs as prepare
...
def test_prepare_creates_byte_exact_read_only_successor_snapshots(
    ...
    try:
        ...
        assert (target / prepare.MANIFEST_NAME).is_file()
...
def test_prepare_rejects_a_source_database_sidecar(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(prepare.PhaseProcessInputError, match="sidecars exist"):
        prepare.prepare_inputs()
