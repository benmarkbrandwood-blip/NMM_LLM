...
class TestB78DBForcedMove(unittest.TestCase):
    ...
    def test_db_forced_move_applied_when_no_capture(self):
        # No capture is generated by d1-d2, so the resolved DB move should be
        # returned.  Keep b2 empty: the old fixture put White on both b2 and
        # f2, which made d1-d2 close b2-d2-f2 and therefore require a capture.
        pos = {p: "" for p in POSITIONS}
        for p in ["d1", "f2", "a7", "g4", "e3"]:
            pos[p] = "W"
        for p in ["d6", "b6", "c3", "b4", "g7"]:
            pos[p] = "B"
        board = BoardState.from_setup(pos, turn="W", phase="move")

        # Verify that the fixture exposes exactly the quiet notation returned
        # by FullGameDB.best_move().
        from game.rules import get_all_legal_moves as legal_moves
        lmoves = legal_moves(board)
        d1_d2 = [
            m for m in lmoves
            if m.get("from") == "d1" and m.get("to") == "d2"
        ]
        self.assertEqual(d1_d2, [{"from": "d1", "to": "d2", "capture": None}])
