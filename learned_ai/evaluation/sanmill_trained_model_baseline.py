...
def audit_instrumentation_surface(root: Path | None = None) -> dict[str, Any]:
    """Audit the single frozen boundary registry.

    The compatibility name is retained for the existing preflight callers,
    but every surface, signature, classification, and evidence expectation is
    now derived from the registry rather than from a hand-written parallel
    inventory.
    """
    ...
    registry, registry_file_sha256 = load_boundary_registry(registry_path)
    report = audit_boundary_registry(registry, repository_root=repository)
    return {
        **report,
        "registry_path": str(registry_path.relative_to(repository)).replace(
            "\\", "/"
        ),
