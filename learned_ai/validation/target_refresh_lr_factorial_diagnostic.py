"""Fail-closed preparation for the target-refresh/LR factorial diagnostic."""
...
CONTRACT_SCHEMA = "nmm.target-refresh-lr-factorial-diagnostic-plan.v1"
SOURCE_READINESS_SCHEMA = (
    "nmm.target-refresh-lr-factorial-diagnostic-source-readiness.v1"
)
READINESS_SCHEMA = "nmm.target-refresh-lr-factorial-diagnostic-readiness.v1"
RESULT_SCHEMA = "nmm.target-refresh-lr-factorial-diagnostic-result.v1"
DEFAULT_CONTRACT = Path(
    "docs/experiments/sanmill-target-refresh-lr-factorial-diagnostic-v1.json"
)
DEFAULT_PATHS_CONFIG = Path("data/training_paths.local.json")
DEFAULT_SOURCE_REPORT = Path(
    "out/target-refresh-lr-factorial-diagnostic-v1/source-readiness.json"
)
DEFAULT_REPORT = Path(
    "out/target-refresh-lr-factorial-diagnostic-v1/readiness.json"
)
...
CONDITION_FACTORS = {
    "refresh-adaptive": (50, "adaptive-search-opponent-win-rate"),
    "refresh-fixed": (50, "fixed"),
    "no-refresh-adaptive": (5001, "adaptive-search-opponent-win-rate"),
    "no-refresh-fixed": (5001, "fixed"),
}


class TargetRefreshLrDiagnosticError(RuntimeError):
    """Raised when the frozen factorial diagnostic does not reconcile."""
...
def _reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise TargetRefreshLrDiagnosticError(f"duplicate JSON key: {key}")
        result[key] = value
    return result
...
def _strict_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise TargetRefreshLrDiagnosticError(
            f"could not load factorial contract: {path}"
        ) from exc
    if not isinstance(value, dict):
        raise TargetRefreshLrDiagnosticError(
            "factorial contract must be an object"
        )
    return value
...
def _schedule_counts(
    ...
    *,
    ...
    for scheduled_index in range(start, stop):
...
def load_target_refresh_lr_contract(path: str | Path) -> dict[str, Any]:
    """Load and fully validate the immutable eight-arm design."""
    contract = _strict_json(Path(path))
    if contract.get("schema_version") != CONTRACT_SCHEMA:
        raise TargetRefreshLrDiagnosticError("unsupported factorial schema")
    ...
    if identity != canonical_sha256(body):
        raise TargetRefreshLrDiagnosticError("factorial plan identity differs")
    if contract.get("status") != "designed_unlaunched_needs_publication":
        raise TargetRefreshLrDiagnosticError("factorial status differs")
    if contract.get("authorization") != {
        ...
        raise TargetRefreshLrDiagnosticError(
            "factorial diagnostic must remain unauthorized"
        )

    arms = contract.get("arms")
    if not isinstance(arms, list) or len(arms) != 8:
        raise TargetRefreshLrDiagnosticError(
            "factorial diagnostic must contain eight arms"
        )
    ...
    if observed != expected:
        raise TargetRefreshLrDiagnosticError("seed or condition order differs")
    if [arm.get("launch_order") for arm in ordered] != list(range(1, 9)):
        raise TargetRefreshLrDiagnosticError("launch order differs")
    for arm in ordered:
        expected_factors = CONDITION_FACTORS[arm["condition"]]
        observed_factors = (
            arm.get("target_refresh_every_games"),
            arm.get("lr_adaptation_mode"),
        )
        if observed_factors != expected_factors:
            raise TargetRefreshLrDiagnosticError(
                f"factor mapping differs: {arm['arm_id']}"
            )
        if arm.get("specialist_read_mode") != "theoretical-only":
            raise TargetRefreshLrDiagnosticError(
                "factorial diagnostic must hold SpecialistDB reads fixed"
            )
    for field in (
        "arm_id",
        "control_dir",
    ...
    if not isinstance(pairing, Mapping):
        raise TargetRefreshLrDiagnosticError("pairing contract is missing")
    if (
    ...
    implementation = (
        analysis.get("result_implementation")
    ...
    if not isinstance(common, Mapping) or not isinstance(resources, Mapping):
        raise TargetRefreshLrDiagnosticError(
            "training resource contract is missing"
        )
    games = resources.get("completed_games_per_arm")
    if (
        games != 100
        or common.get("one_segment_games") != 100
        or resources.get("maximum_completed_games_total") != 800
        or resources.get("active_wall_hours_per_arm") != 0.25
        ...
        raise TargetRefreshLrDiagnosticError("training resource values differ")
    if games >= common["fixed_resource_stage_games"][0]:
        raise TargetRefreshLrDiagnosticError(
            "factorial diagnostic must stay at node level one"
        )
    schedules = resources.get("schedule_counts_by_seed")
    if not isinstance(schedules, Mapping):
        raise TargetRefreshLrDiagnosticError("schedule counts are missing")
    sanmill_games_per_arm = 0
    for seed in EXPECTED_SEEDS:
        expected_schedule = {
            "all": _schedule_counts(
                seed,
                start=0,
                stop=100,
                frozen_ratio=common["frozen_target_ratio"],
            ),
            "pre_boundary": _schedule_counts(
                seed,
                start=0,
                stop=50,
                frozen_ratio=common["frozen_target_ratio"],
            ),
            "post_boundary": _schedule_counts(
                seed,
                start=50,
                stop=100,
                frozen_ratio=common["frozen_target_ratio"],
        ...
        if schedules.get(str(seed)) != expected_schedule:
            raise TargetRefreshLrDiagnosticError(
                f"seed {seed} schedule differs"
            )
        all_counts = expected_schedule["all"]
        sanmill_games_per_arm += (
            all_counts["sanmill_black"] + all_counts["sanmill_white"]
        )
    requested_nodes = (
        sanmill_games_per_arm
    ...
    if resources.get("maximum_requested_sanmill_nodes_total") != requested_nodes:
        raise TargetRefreshLrDiagnosticError("Sanmill node ceiling differs")
    return contract
...
def build_prepare_commands(
    *,
    ...
    """Return all eight frozen preparation commands in launch order."""
    return [
...
def _command_training_semantics(
    command: list[str],
    *,
    ignore_seed: bool,
    ignore_factors: bool,
    ...
    if ignore_seed:
        ignored.add("seed")
    if ignore_factors:
        ignored.update({"update_target_every", "lr_adaptation_mode"})
    semantics = {
        key: value
...
def validate_prepare_commands(
    ...
    """Prove that generated commands differ only by seed and two factors."""
    if len(commands) != 8:
        raise TargetRefreshLrDiagnosticError("prepare command count differs")
    base_semantics: set[str] = set()
    condition_semantics: dict[str, set[str]] = {
        condition: set() for condition in EXPECTED_CONDITIONS
    }
    for arm, command in zip(_ordered_arms(contract), commands, strict=True):
        if "authorize" in command or "run-next" in command:
            raise TargetRefreshLrDiagnosticError("preparation command can train")
        args = manager._build_parser().parse_args(command[2:])
        if args.seed != arm["seed"]:
            raise TargetRefreshLrDiagnosticError("command seed differs")
        if args.target_refresh_every != arm["target_refresh_every_games"]:
            raise TargetRefreshLrDiagnosticError(
                "command target-refresh factor differs"
            )
        if args.lr_adaptation_mode != arm["lr_adaptation_mode"]:
            raise TargetRefreshLrDiagnosticError(
                "command learning-rate factor differs"
            )
        if args.specialist_read_mode != "theoretical-only":
            raise TargetRefreshLrDiagnosticError("command read mode differs")
        condition_semantics[arm["condition"]].add(
            _command_training_semantics(
                command,
                ignore_seed=True,
                ignore_factors=False,
            )
        )
        base_semantics.add(
            _command_training_semantics(
    ...
    if any(len(values) != 1 for values in condition_semantics.values()):
        raise TargetRefreshLrDiagnosticError(
            "same-condition commands differ outside seed"
        )
    if len(base_semantics) != 1:
        raise TargetRefreshLrDiagnosticError(
            "commands differ outside seed and frozen factors"
        )
...
def _inspect_source(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if branch != "dev":
        raise TargetRefreshLrDiagnosticError("source audit requires dev")
    if status:
        raise TargetRefreshLrDiagnosticError(
            "source audit requires a clean tracked and untracked worktree"
        )
    if origin_main != contract["lineage"]["main_review"]["reviewed_tip"]:
        raise TargetRefreshLrDiagnosticError("origin/main moved after review")
...
def inspect_result_implementation(
    ...
    """Verify that the preregistered result code is tracked and unchanged."""
    ...
    for name in ("module", "publisher"):
        ...
        if not path.is_file() or not _tracked_file(root, path):
            raise TargetRefreshLrDiagnosticError(f"result {name} is not tracked")
        observed = _sha256_file(path)
...
def _assert_plan_semantics(
    ...
    *,
    ...
    if (
        ...
        raise TargetRefreshLrDiagnosticError("policy-health contract differs")
    return args
...
def audit_prepared_plans(
    *,
    ...
    if len(base_semantics) != 1:
        raise TargetRefreshLrDiagnosticError(
            "prepared arms differ outside seed and frozen factors"
        )
    return audited
...
def inspect_source_readiness(
    *,
    ...
    if not unresolved:
        unresolved.append(
            "generate eight immutable plans and preflights; training remains unauthorized"
        )
    body = {
...
def prepare_target_refresh_lr_diagnostic(
    *,
    root: Path,
    contract_path: Path,
    paths_config: Path,
    ...
    """Create eight plans and preflights without authorization or training."""
    root = root.resolve()
    contract_path = contract_path.resolve(strict=True)
    paths_config = paths_config.resolve(strict=True)
    ...
    source = _inspect_source(root, contract)
    if not source["published"]:
        raise TargetRefreshLrDiagnosticError("dev must equal origin/dev")
    evidence = _inspect_source_evidence(root, contract)
...
__all__ = [
    ...
    "prepare_target_refresh_lr_diagnostic",
