...
def _start_level_horizon_precision(
    records: list[dict[str, Any]],
) -> dict[str, Any]:
    """Cluster the paired horizon-survival difference by independent start."""
    by_colour_unit: dict[tuple[str, str], dict[str, float]] = {}
    for record in records:
        survives = record.get("ongoing_after_total_logical_ply_120")
        if not isinstance(survives, bool):
            continue
        key = (str(record["source_core_id"]), str(record["candidate_color"]))
        by_colour_unit.setdefault(key, {})[str(record["candidate_id"])] = float(
            survives
        )
    ...
    fixed_width_budgets = []
    if deviation is not None:
        for target_half_width in (0.10, 0.075, 0.05):
            starts = math.ceil((1.96 * deviation / target_half_width) ** 2)
            fixed_width_budgets.append(
                {
                    "target_half_width": target_half_width,
    ...
    return {
        ...
        "interpretation": (
            "fixed reused-development-corpus engineering interval at the start "
            "cluster level; a planning input for a new-corpus process study, "
            "not a population guarantee or playing-strength claim"
        ),
        "strength_claim_allowed": False,
    }
...
def build_payload(output_root: str | Path) -> dict[str, Any]:
    ...
    return {
        ...
        "horizon_precision_by_start": _start_level_horizon_precision(records),
...
function horizonPrecision(s){if(!s||!s.start_units_complete)return `<div class="panel"><h2>起点层 120 手存活精度</h2><p class="help">等待同一起点的黑白两色都形成完整 v3/v4 horizon 配对。</p></div>`;const iv=s.interval||[null,null],dist=Object.entries(s.distribution||{}).sort((a,b)=>Number(a[0])-Number(b[0])).map(([value,count])=>`<tr><td>${pp(Number(value))}</td><td>${intval(count)}</td></tr>`).join(''),budget=(s.fixed_width_budgets||[]).map(x=>`<tr><td>${pp(x.target_half_width)}</td><td>${intval(x.starts)}</td><td>${intval(x.games)}</td></tr>`).join('');return `<div class="panel"><h2>起点层 120 手存活精度（零新对局）</h2><p class="help">每个独立起点先平均候选执白与执黑的两个 v4−v3 存活差，再在起点之间计算工程区间。这样保留黑白交换的配对结构；它只为新语料过程实验估算固定半宽，不能把“更能拖过观察窗”解释成更强。</p><div class="grid">${card('完整起点',`${s.start_units_complete} / ${s.start_units_expected}`,`${s.matched_colour_units} 个颜色配对`)}${card('起点层均值 v4 − v3',pp(s.mean),`工程区间 ${pp(iv[0])} … ${pp(iv[1])}`)}${card('起点层标准差',pp(s.sample_standard_deviation),`区间半宽 ${pp(s.half_width)}`)}${card('声明边界','仅过程规划','新语料的实际方差仍可能更大')}</div><div class="two"><div><h2>起点差值分布</h2><table><thead><tr><th>两色平均存活差</th><th>起点数</th></tr></thead><tbody>${dist}</tbody></table></div><div><h2>固定半宽预算（语料内估计）</h2><table><thead><tr><th>目标半宽</th><th>起点</th><th>总局数</th></tr></thead><tbody>${budget}</tbody></table><p class="help">每个新起点需要两候选 × 黑白交换，共 4 局。预算复用的是本开发语料的起点层标准差，不是对新语料精度的保证；正式合同仍须冻结语料、窗口、失败规则和资源上限。</p></div></div></div>`}
...
${horizonPrecision(p.horizon_precision_by_start)}
