#!/usr/bin/env python3
"""Create or audit successor-owned immutable phase-process input snapshots."""
...
MANIFEST_SCHEMA = "nmm.retained-phase-process-input-snapshots.v1"
...
CANDIDATES = {
    "retained-v3-refresh50": {
        ...
        "source_specialist_db": _ROOT
        / "data/specialist_db.sanmill_preserving_retained_v3.seed58."
        "audit_snapshot.sqlite",
    ...
    "retained-v4-no-refresh": {
        ...
        "source_specialist_db": SOURCE_ROOT
...
class PhaseProcessInputError(RuntimeError):
    """Raised when a source or successor snapshot differs."""
...
def _relative(path: Path) -> str:
    try:
        ...
        raise PhaseProcessInputError("snapshot path leaves the repository") from exc
...
def _file_record(path: Path, *, base: Path) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise PhaseProcessInputError("snapshot input must be a regular file")
    return {
...
def _bundle_files(path: Path) -> list[dict[str, Any]]:
    if path.is_symlink() or not path.is_dir():
        raise PhaseProcessInputError("route bundle directory is absent")
    files = sorted(item for item in path.rglob("*") if item.is_file())
    if any(item.is_symlink() for item in path.rglob("*")):
        raise PhaseProcessInputError("route bundle contains a symbolic link")
...
def _assert_no_sqlite_sidecars(path: Path) -> None:
    ...
    if sidecars:
        raise PhaseProcessInputError(
            "SpecialistDB sidecars exist: "
...
def _candidate_record(
    ...
    *,
    ...
    if verified.get("bundle_identity") != config["bundle_identity"]:
        raise PhaseProcessInputError(f"{candidate_id} route identity differs")
    ...
    if specialist_hash != config["specialist_db_sha256"]:
        raise PhaseProcessInputError(f"{candidate_id} SpecialistDB differs")
    return {
        ...
        "specialist_db": {
            "path": _relative(
                TARGET_ROOT / str(config["target_specialist_db"])
            ),
            "bytes": specialist_db.stat().st_size,
            "sha256": specialist_hash,
            "sidecars_absent": True,
            "read_only_file": not bool(
                specialist_db.stat().st_mode & stat.S_IWRITE
            ),
        },
    }
...
def build_manifest(root: Path | None = None) -> dict[str, Any]:
    ...
    if root.resolve() != TARGET_ROOT.resolve():
        raise PhaseProcessInputError("snapshot root differs")
    if not root.is_dir():
        raise PhaseProcessInputError("successor snapshot root is absent")
    candidates = [
    ...
    if manifest_path.is_file():
        ...
        try:
            ...
            raise PhaseProcessInputError("snapshot manifest is invalid") from exc
        if canonical_json_bytes(persisted) != raw or persisted != result:
            raise PhaseProcessInputError("snapshot manifest differs")
    return result
...
def prepare_inputs() -> dict[str, Any]:
    ...
    temporary = Path(
        tempfile.mkdtemp(prefix=".phase-process-inputs-", dir=TARGET_ROOT.parent)
    )
    try:
        for config in CANDIDATES.values():
            ...
            if _sha256_file(source_db) != config["specialist_db_sha256"]:
                raise PhaseProcessInputError("source SpecialistDB differs")
            shutil.copytree(
                source_bundle,
            ...
            shutil.copy2(
                source_db,
                temporary / str(config["target_specialist_db"]),
            )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="prepare once or audit the already prepared immutable snapshots",
    )
    return parser
