#!/usr/bin/env python3
"""tools/placement_puzzle_generator.py — CLI for generating NMM placement-phase puzzles.

Uses the Malom ultra-strong perfect database to find placement-phase positions
with a forced win in 4–7 moves for the current mover.

The generator samples positions mid-placement (2–6 pieces still to place per
side) and verifies a forced win using greedy Malom WDL extraction.  Only
positions without existing mills are considered (mills imply a capture has
already occurred, which is rejected for sampling integrity).
...
Usage examples:
  ...
  .venv/bin/python tools/placement_puzzle_generator.py --count 30

  # Win-in-5, Black to win
  .venv/bin/python tools/placement_puzzle_generator.py --depth 5 --side B --count 20

  # Unique-move only (max 1 winning first move)
  .venv/bin/python tools/placement_puzzle_generator.py --max-winning-moves 1 --count 20
"""
...
from ai.malom_puzzle_search import generate_malom_placement_puzzle, prewarm_hash_cache

_OUT_DIR = _ROOT / "data" / "puzzles" / "placement"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="NMM placement-phase puzzle generator (Malom perfect database)",
    ...
    print("Pre-warming Malom hash states for pieces 3–7…", end=" ", flush=True)
    t_warm = time.time()
