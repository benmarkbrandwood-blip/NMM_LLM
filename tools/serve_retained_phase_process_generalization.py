...
def _heldout_score_payload(
    ...
    return {
        ...
        "by_candidate_color": report["by_candidate_color"],
        "by_phase": report["by_phase"],
        "paired_secondary": {
            "survival_108_post_start": report["paired"][
                "start_clustered_108_ply_survival_v4_minus_v3"
            ],
            "restricted_length": report["paired"][
                "start_clustered_restricted_length_v4_minus_v3"
            ],
        },
...
<title>NMM_LLM · retained-v3/v4 评测监控</title>
<style>
...
<body><main id="app"><div class="empty"><div class="panel"><h1>正在读取 retained-v3/v4 评测证据…</h1></div></div></main>
<script>
...
function render(payload){const app=document.getElementById('app');if(!payload.available){app.innerHTML=`<div class="empty"><div class="panel"><h1>v3/v4 评测监控</h1><p class="sub">${esc(payload.message)}</p><div class="notice">没有精确计划和授权时，网页只显示“未启动”，不会预填或推测结果。</div></div></div>`;return}const r=payload.report,a=r.by_candidate[C.v3],b=r.by_candidate[C.v4],p=r.paired.primary_start_clustered_108_ply_survival_v4_minus_v3,iv=p.interval||[null,null],active=payload.progress.active_seconds,hs=payload.heldout_score,heldoutCurrent=hs&&hs.available,pageTitle=heldoutCurrent?'NMM_LLM · retained-v3 / no-refresh-v4 held-out 高精度得分评测':'NMM_LLM · retained-v3 / no-refresh-v4 阶段过程确认',pageStatus=heldoutCurrent?hs.status:payload.status,pageNotice=heldoutCurrent?'<b>当前主结果是固定 held-out 语料上的命名 route 得分关系。</b> 不能归因 refresh，不能外推 Elo/总体棋力，也不会自动触发晋级、发布或释放。':'<b>固定项目可见语料的过程确认，不是 held-out 棋力评测。</b> 结果不能归因 refresh，不能用于晋级、发布或释放。';
app.innerHTML=`<h1>${pageTitle}</h1><div class="sub">${heldoutCurrent?esc(hs.plan_identity):esc(payload.identities.diagnostic_id)} · <span class="badge">${esc(pageStatus)}</span></div><div class="notice">${pageNotice}</div>
${heldoutScoreBlock(payload.heldout_score)}
${researchDispositionBlock(payload.heldout_score)}
<div class="panel"><h2>历史阶段过程诊断（156 / 156，已完成）</h2><p class="help">下面的完成进度、39 个阶段起点和相对 108 手主差值属于上一轮项目可见语料诊断，不是当前 held-out 评测进度。</p></div>
...
function researchDispositionBlock(x){const selected=x&&x.result_identity==='8d7a4a0aefdd9b0716cccfa3a8d9ace44493c870cc9c9eed885bc7fd35c74730'&&x.primary&&x.primary.decision==='v4_higher_fixed_heldout_score';if(!selected)return '';return `<div class="panel"><h2>产品处置：retained-v4 是优选研究候选</h2><p class="help">产品负责人已将精确的 <code>retained-v4-no-refresh</code> route bundle <code>817d2e36…</code> 选为后续研究规划的优选候选；retained-v3 保留为冻结对照。这个处置没有复制、改名或替换任何模型，也不是晋级、部署、发布或释放，不授权新对局、训练或更新。</p></div>`}
function heldoutScoreBlock(x){if(!x)return '';if(!x.available)return `<div class="panel"><h2>held-out 高精度得分方案</h2><p class="help">${esc(x.message||'计划尚未冻结。')}</p></div>`;const p=x.primary,iv=p&&p.interval?p.interval:[null,null],a=x.by_candidate&&x.by_candidate[C.v3],b=x.by_candidate&&x.by_candidate[C.v4],phase=x.phase_counts||{},dist=p&&p.distribution?p.distribution:{},directions=Object.entries(dist).reduce((acc,[value,count])=>{if(Number(value)>0)acc.v4+=count;else if(Number(value)<0)acc.v3+=count;else acc.tie+=count;return acc},{v4:0,v3:0,tie:0}),secondary=x.paired_secondary||{},survival=secondary.survival_108_post_start,survivalIv=survival&&survival.interval?survival.interval:[null,null],groupRows=(source,order,labels)=>order.map(key=>{const group=source&&source[key],v3=group&&group[C.v3]&&group[C.v3].eventual_rules_wdl,v4=group&&group[C.v4]&&group[C.v4].eventual_rules_wdl;return `<tr><td>${labels[key]}</td><td>${v3?integer(v3.support):'—'}</td><td>${v3?pct(v3.score_rate):'—'}</td><td>${v4?pct(v4.score_rate):'—'}</td><td>${v3&&v4?pp(v4.score_rate-v3.score_rate):'—'}</td></tr>`}).join(''),colourRows=groupRows(x.by_candidate_color,['W','B'],{W:'候选执白',B:'候选执黑'}),heldoutPhaseRows=groupRows(x.by_phase,['placement','movement','flying'],{placement:'placement',movement:'movement',flying:'flying'}),wdlRows=[[a,'v3'],[b,'v4']].map(([candidate,label])=>{const w=candidate&&candidate.eventual_rules_wdl;return `<tr><td>${label}</td><td>${w?integer(w.support):'—'}</td><td>${w?integer(w.wins):'—'}</td><td>${w?integer(w.draws):'—'}</td><td>${w?integer(w.losses):'—'}</td><td>${w?pct(w.score_rate):'—'}</td><td>${w?integer(w.safety_cap_excluded):'—'}</td></tr>`}).join('');return `<div class="panel"><h2>held-out 高精度得分评测：253 起点 / 1,012 局 / ±1.5pp</h2><p class="help">每个独立起点由 v3、v4 各执白和执黑共四局组成；先在起点内平均两种颜色的 v4−v3 得分差，再跨 253 个起点计算区间。胜/和/负按 1 / 0.5 / 0 计分。</p><div class="grid">${card('状态',esc(x.status),x.authorization_present?(x.authorization_consumed?'授权已消费':'授权文件已存在，尚未消费'):'尚无启动授权')}${card('完成进度',`${integer(x.completed_games)} / ${integer(x.expected_games)}`,`活动上限 ${num(x.max_active_hours,1)} h`)}${card('独立起点',integer(x.selected_starts),`P ${integer(phase.placement)} · M ${integer(phase.movement)} · F ${integer(phase.flying)}`)}${card('目标半宽',pp(x.maximum_primary_half_width),'95% 起点聚类工程区间')}${card('主得分差 v4−v3',p?pp(p.mean):'—',p&&iv[0]!=null?`${pp(iv[0])} … ${pp(iv[1])}`:'尚未产生评测对局')}${card('主判决',p?heldoutScoreDecision(p.decision):'等待授权',p?`实际半宽 ${pp(p.half_width)}；支持 ${integer(p.support)} 起点`:'计划冻结不等于可以启动')}${card('起点内方向',p?`${integer(directions.v4)} / ${integer(directions.v3)} / ${integer(directions.tie)}`:'—','v4 领先 / v3 领先 / 同分')}${card('108 手存活差',survival?pp(survival.mean):'—',survival&&survivalIv[0]!=null?`${pp(survivalIv[0])} … ${pp(survivalIv[1])}；次要`:'等待完整起点')}</div><div class="two"><div><h2>严格终局 W/D/L</h2><table><thead><tr><th>候选</th><th>局数</th><th>胜</th><th>和</th><th>负</th><th>计分率</th><th>cap</th></tr></thead><tbody>${wdlRows}</tbody></table></div><div><h2>按候选颜色分层（描述性）</h2><table><thead><tr><th>颜色</th><th>每候选局数</th><th>v3 计分</th><th>v4 计分</th><th>差值</th></tr></thead><tbody>${colourRows}</tbody></table></div></div><div class="panel"><h2>按冻结起始阶段分层（描述性）</h2><table><thead><tr><th>阶段</th><th>每候选局数</th><th>v3 计分</th><th>v4 计分</th><th>差值</th></tr></thead><tbody>${heldoutPhaseRows}</tbody></table><p class="help">颜色与阶段行没有单独预注册方向区间，只用于定位主差值，不能独立升级为结论。</p></div><p class="help"><b>怎么读：</b>区间完全高于 0 且半宽不超过 1.5pp，才判 v4 在这批冻结 held-out 起点上得分更高；完全低于 0 才判 v3 更高；跨 0 只能判“不确定”，绝不等同于“两模型等效”。若任何对局命中 1,536 post-start 安全 cap，该局不按和棋计，方向性主结论 fail closed。108 手存活、无吃子计时、重复、阶段和 Malom 指标都是解释性过程指标，不能替换配对得分主指标。该结果也不能隔离 target refresh 因果、外推 Elo/总体棋力或自动触发晋级、发布、释放。</p><code>plan ${esc(x.plan_identity)}${x.spec_identity?` · spec ${esc(x.spec_identity)}`:''}${x.result_identity?` · result ${esc(x.result_identity)}`:''}</code></div>`}
"""
