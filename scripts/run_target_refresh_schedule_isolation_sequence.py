...
SEQUENCE_READINESS_SCHEMA = (
    "nmm.target-refresh-schedule-isolation-sequence-readiness.v1"
)
SEQUENCE_LAUNCH_SCHEMA = "nmm.target-refresh-schedule-isolation-launch.v1"
SEQUENCE_RESULT_SCHEMA = "nmm.target-refresh-schedule-isolation-sequence-result.v1"
SEQUENCE_FAILURE_SCHEMA = (
    "nmm.target-refresh-schedule-isolation-sequence-failure.v1"
)
...
def _publish_exclusive(path: Path, value: Mapping[str, Any]) -> None:
    ...
    if target.exists():
        raise ScheduleIsolationSequenceError(f"output already exists: {target}")
    try:
...
def inspect_sequence_readiness(
    *,
    ...
    body = {
        ...
        "launch_authorized": authorization_path.exists(),
    }
    return {**body, "sequence_readiness_identity": canonical_sha256(body)}
...
def record_sequence_authorization(
    *,
    ...
    contract = load_equal_transition_contract(contract_path)
    readiness = _strict_json(readiness_path)
    observed = validate_readiness_identity(readiness)
    if observed != expected_readiness_identity:
        raise ScheduleIsolationSequenceError("expected readiness identity differs")
...
def launch_sequence(
    *,
    ...
    contract = load_equal_transition_contract(contract_path)
    readiness = _strict_json(readiness_path)
    if validate_readiness_identity(readiness) != expected_readiness_identity:
        raise ScheduleIsolationSequenceError("expected readiness identity differs")
    ...
    marker_body = {
        ...
        "sequence_readiness_identity": readiness_report[
