"""scripts/train_s_gen_v3.py — Generalist v3: diff-windowed SpecialistDB + inference parity.

Extends train_s_gen_v3.py with three targeted changes:

  1. Diff-windowed SpecialistDB queries: the AI only consults SpecialistDB
     positions tagged from the last ``--sdb-diff-window`` (default 5) played
     difficulty levels.  Writes go to both ``positions`` (aggregate, used at
     inference) and ``positions_by_diff`` (per-level, used for windowed reads).
     Uses a separate DB path (specialist_db_v3.sqlite) to avoid touching v2c's
     data.

  2. Fixed sim_ply_depth: LookaheadAdvisor simulation depth is no longer
     scaled with difficulty.  It stays at ``--sim-ply-depth`` (default 12)
     throughout training to match the inference default of 12, eliminating the
     training/inference feature-quality mismatch observed with v2c.

  3. Diff-20 completion gate: promotion at diff_max requires a score of 0.90
     (up from ~0.60) to declare training done.

All else identical to v2c (DRAW_REP_PENALTY, REVISIT_PENALTY, Malom rewards,
draw cap, NOVELTY_BONUS).
...
Usage
-----
.venv/bin/python scripts/train_s_gen_v3.py --max-games 20
.venv/bin/python scripts/train_s_gen_v3.py --auto-resume-best
"""
...
STAGE_TAG = "s_gen_v3"
OUT_DIR   = "learned_ai/checkpoints/scaffolded/s_gen_v3"
...
def _load_model(
    ...
    _valid_lineage_tags = {STAGE_TAG, "s_gen_v2a", "s_gen_v2b", "s_gen_v2c"}
...
def main() -> None:
    ...
    p.add_argument("--out-dir",   default=str(_ROOT / "learned_ai" / "checkpoints" / "scaffolded" / "s_gen_v3"))
    ...
    p.add_argument("--specialist-db", default=str(_ROOT / "data" / "specialist_db_v3.sqlite"),
                   help="Path to SpecialistDB SQLite file (v3 default: specialist_db_v3.sqlite)")
