#!/usr/bin/env python3
"""Audit or prepare the target-refresh/LR factorial diagnostic."""
...
from learned_ai.validation.target_refresh_lr_factorial_diagnostic import (  # noqa: E402
    DEFAULT_CONTRACT,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.prepare:
            if args.write_source_readiness:
                parser.error(
                    "--prepare and --write-source-readiness are exclusive"
                )
            report = prepare_target_refresh_lr_diagnostic(
                root=ROOT,
    ...
    except (TargetRefreshLrDiagnosticError, OSError, ValueError) as exc:
        print(
            json.dumps(
                {
                    "state": "not_ready",
