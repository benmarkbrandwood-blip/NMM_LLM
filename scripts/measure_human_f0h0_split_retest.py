"""Run the corrected read-only F0-H0 split-feasibility measurement."""
...
DEFAULT_PLAN = Path(
    "docs/experiments/f0-h0-corrected-split-feasibility-measurement-v1.json"
)
DEFAULT_RESULT = Path(
    "docs/evidence/"
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Measure three corrected F0-H0 split candidates without selection."
    )
...
def _execute(args: argparse.Namespace) -> dict[str, Any]:
        ...
        def progress(current: int, total: int) -> None:
            if current == total or current % 2_500 == 0:
                print(
                    json.dumps(
                        {
                            "stage": "design_c_strict_ring16_replay",
    ...
    if result["lineage"]["f0d0_manifest_identity"] != boundary.manifest_identity:
    ...
    if any(
        ...
        raise SplitRetestError("measurement result crossed a prohibited boundary")
    return {
        "schema_version": RESULT_SCHEMA,
        "verified": True,
        "result_identity": result["result_identity"],
