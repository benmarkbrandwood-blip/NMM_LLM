#!/usr/bin/env python3
"""Run the frozen 1,024-game extended exploration sample only."""
...
def _malom_path(config_path: Path) -> Path:
    try:
        ...
        raise FeatureDeviationError("local path configuration unavailable") from exc
    raw = value.get("malom_db_path") if isinstance(value, dict) else None
    if not isinstance(raw, str) or not raw:
        raise FeatureDeviationError("malom_db_path absent")
    path = Path(raw)
...
def main() -> int:
    ...
    try:
        plan, _plan_file_sha = load_design_plan(_ROOT / args.plan)
        split, _split_file_sha = load_split_v2(_ROOT / args.split)
        ...
        payload = run_extended_exploration(
