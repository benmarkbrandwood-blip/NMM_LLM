"""scripts/train_s_gen_v4.py — Generalist v4: full game recording (both sides).

Extends train_s_gen_v3.py with one targeted change:

  Full game DB recording: SpecialistDB now stores board positions and move
  sequences from BOTH the learner and the opponent, not just the learner.

  Previously the AI only accumulated statistics for positions where it was to
  move.  In v4 every board state in a game — regardless of whose turn it is —
  is written to the DB with WDL from that player's perspective.  Additionally,
  when the opponent wins the opponent's move sequence is saved as a winning_line
  so the AI can learn from those patterns if it ever reaches the same positions.

  Uses a separate DB path (specialist_db_v4.sqlite) to keep v3 data intact.
  Checkpoint directory: learned_ai/checkpoints/scaffolded/s_gen_v4.

All else identical to v3 (diff-windowed SDB, fixed sim_ply_depth, diff-20 gate,
DRAW_REP_PENALTY, REVISIT_PENALTY, Malom rewards, draw cap, NOVELTY_BONUS).

Opponent schedule (per game):
  10% — next higher difficulty (anti-overfit)
  20% — random lower difficulty
  10% — blundering heuristic (25% blunder rate)
...
Usage
-----
.venv/bin/python scripts/train_s_gen_v4.py --max-games 20
.venv/bin/python scripts/train_s_gen_v4.py --auto-resume-best
"""
...
STAGE_TAG = "s_gen_v4"
OUT_DIR   = "learned_ai/checkpoints/scaffolded/s_gen_v4"

FEATURE_SCHEMA_VERSION = "nmm.scaffolded-generalist.v2a"
LABEL_SCHEMA_VERSION   = "sector-corrected-v1"
BEST_CHECKPOINT_MIN_GAMES = 10

# v4: only record position stats from this ply onwards so opening moves aren't
...
DIFF_MAX_COMPLETION_TARGET = 0.90   # score% required to declare "done" at diff_max

# ── Diff-windowed SpecialistDB proxy ─────────────────────────────────────────


class _DiffWindowDB:
    """Proxy that tags writes with a diff level while reading from the full DB.

    Reads route to the aggregate ``positions`` table (all difficulty levels),
    so the AI sees its complete self-play history at every difficulty.
    Writes go to both ``positions`` (inference-compatible) and
    ``positions_by_diff`` (tagged with current difficulty for future analysis).

    Passed as ``specialist_db`` to ``_rollout``; calling code never touches
    the underlying ``SpecialistDB`` directly for query or write during rollout.
    """

    def __init__(self, db: "SpecialistDB", min_diff: int, max_diff: int) -> None:
        self._db = db
        self._min_diff = min_diff
        self._max_diff = max_diff

    def require_writable(self) -> None:
        self._db.require_writable()

    def query_wdl_evidence(self, board, min_samples: int = 5):
        return self._db.query_wdl_evidence(board, min_samples)

    def query_wdl(self, board, min_samples: int = 5):
        return self._db.query_wdl(board, min_samples)

    def record_game(
        self, boards, result, move_seq, phase, learner_color=None
    ) -> None:
        # Aggregate write (inference reads this) + diff-tagged write (training window).
        self._db.record_game(boards, result, move_seq, phase, learner_color)
        self._db.record_game_diff(boards, result, move_seq, phase, learner_color,
                                   diff_level=self._max_diff)

    def label_position_malom(self, board, wdl: str) -> None:
        self._db.label_position_malom(board, wdl)
...
# v2c: deliberate repetition draws penalised much harder than passive draws.
# Not in (DRAW_SHORT, DRAW_LONG) so _retroactive_rescore bypasses draw_penalty_scale
...
# v2c: directional Malom rewards — only fire on WDL *transitions*, not static position.
MALOM_PROGRESS_REWARD  =  0.15   # D/L → W: genuine winning progress
MALOM_REGRESS_PENALTY  = -0.10   # W → D/L: moving away from a won position

# v2c: per-move revisit deterrent and novelty bonus (both fire inline in _rollout).
REVISIT_PENALTY = -0.05   # learner revisits a board state already seen this game
NOVELTY_BONUS   =  0.02   # learner visits a brand-new board state (not seen this game)
...
class RewardBreakdown:
    ...
    retro:       float = 0.0
    malom:       float = 0.0
    malom_dir:   float = 0.0   # v2c: directional Malom (transition-based)
    revisit:     float = 0.0   # v2c: per-move revisit penalty
    novelty:     float = 0.0   # v2c: per-move novelty bonus
...
def _run_s1b_refresher(
    ...
    p = Path(data_path)
    if not p.exists():
        print(f"[s_gen_v3] s1b refresher: data not found ({data_path}) — skipping")
        return
    ...
    model.train()
    print(f"[s_gen_v3] s1b refresher: loser={len(loser_idxs)} winner={len(winner_idxs)} positions  lr={lr:.2e}")
    ...
    def _run_phase(phase_idxs: list[int], phase_label: str, use_heuristic_target: bool) -> None:
        ...
        for epoch in range(1, epochs + 1):
            ...
            print(f"[s_gen_v3]   refresher [{phase_label}] epoch {epoch}/{epochs}  loss={ep_loss / max(ep_w_sum, 1e-9):.4f}")
    ...
    model.eval()
    print("[s_gen_v3] s1b refresher done")
...
def _derive_game_identity(run_seed: int, scheduled_index: int, role: str) -> tuple[str, int]:
    """Stable game ID + CPU Torch seed from immutable inputs (no external deps)."""
    raw = json.dumps(
        {"schema": "nmm.game-identity.v1", "run_seed": run_seed,
         "scheduled_index": scheduled_index, "role": role},
        sort_keys=True,
    ).encode()
    h = hashlib.sha256(raw).hexdigest()
    return f"game:{h}", int(h[:16], 16) & ((1 << 63) - 1)
...
def _load_model(
    ...
    if ckpt_hidden != policy_hidden:
        print(f"[s_gen_v3] policy_hidden mismatch: ckpt={ckpt_hidden} vs requested={policy_hidden} — starting fresh")
        return _fresh()
    ...
    try:
        ...
        try:
            ...
            print("[s_gen_v3] Warning: value_mlp shape mismatch — policy weights loaded, value head reinitialized")
        except RuntimeError:
            print(f"[s_gen_v3] State dict incompatible — starting fresh with policy_hidden={policy_hidden}")
            return _fresh()
    stage          = ckpt.get("stage", "unknown")
    # Accept v2a, v2b, and v2c tags as valid lineage — v3 is the continuation
    # of that codebase.  Keeps existing checkpoints resumable without editing.
    _valid_lineage_tags = {STAGE_TAG, "s_gen_v3", "s_gen_v2a", "s_gen_v2b", "s_gen_v2c"}
...
def _should_save_best_checkpoint(
    ...
    return heuristic_game_count >= BEST_CHECKPOINT_MIN_GAMES and win_rate > best_win_rate_at_diff


def _report_final_checkpoints(out_dir: Path) -> None:
    latest_path = out_dir / "latest.pt"
    best_path   = out_dir / "best.pt"
    print(f"[s_gen_v3] Latest checkpoint: {latest_path}")
    if best_path.exists():
        print(f"[s_gen_v3] Best checkpoint available: {best_path}")
    else:
        print(
            f"[s_gen_v3] Best checkpoint: not created "
...
def _compute_per_move_reward(
    ...
    malom_before: Optional[str] = None,
    ...
    # v2c: static Malom reward zeroed (MALOM_REWARD=0) — replaced by directional signal.
    if malom_q == "W":
        rb.malom = MALOM_REWARD
    elif malom_q == "L":
        rb.malom = -MALOM_REWARD

    # v2c: directional Malom — reward transitions that improve WDL, penalise regressions.
    if malom_before is not None and malom_q is not None:
        if malom_before in ("D", "L") and malom_q == "W":
            rb.malom_dir = MALOM_PROGRESS_REWARD
        elif malom_before == "W" and malom_q in ("D", "L"):
            rb.malom_dir = MALOM_REGRESS_PENALTY

    rb.total = rb.sentinel + rb.heuristic + rb.malom + rb.malom_dir
...
def _mk_blend_weights():
    """Return HeuristicWeights for blended-opponent games: VN 10%, gap 30%."""
    import dataclasses
...
def _rollout(
    ...
    opponent_boards: list[BoardState] = []
    opponent_result_boards: list[BoardState] = []
    opponent_moves_notation: list[str] = []
        ...
        if player == learner_color:
            ...
            with torch.no_grad():
                ...
                if _total.item() <= 1e-9:
                    ...
                    logging.warning(
                        "[s_gen_v3] non-finite logits at ply=%d "
            ...
            if not opp_move:
                ...
                opponent_boards.pop()  # remove speculative board on infra exit
                outcome = WIN_REWARD
            ...
            opponent_moves_notation.append(_move_notation(opp_move))  # track for winning_lines
            _opp_captured = bool(opp_move.get("capture"))
            board = board.apply_move(opp_move)
            opponent_result_boards.append(board)  # record post-move board
    ...
    if specialist_db is not None and not _is_infra_final:
        try:
            _res = "W" if outcome == WIN_REWARD else ("L" if outcome == LOSS_REWARD else "D")
            _opp_color = "B" if learner_color == "W" else "W"
            # v4: split boards at SDB_PLY_THRESHOLD.  Sides alternate, so the
            # first (SDB_PLY_THRESHOLD // 2) entries in each per-side list
            # correspond to plies 0..(SDB_PLY_THRESHOLD-1).
            _ei = SDB_PLY_THRESHOLD // 2   # early index cutoff per side
            _early_boards = (learner_boards[:_ei] + learner_result_boards[:_ei] +
                             opponent_boards[:_ei] + opponent_result_boards[:_ei])
            _late_boards  = (learner_boards[_ei:] + learner_result_boards[_ei:] +
                             opponent_boards[_ei:] + opponent_result_boards[_ei:])
            # Early boards: record as neutral — position is visited but the
            # game outcome is not attached, so opening moves are never penalised
            # for a loss that happened later in the game.
            if _early_boards:
                specialist_db.record_game(
                    _early_boards, "D", [], "gen",
                    learner_color=learner_color,
                )
            # Late boards: real outcome.  move_seq and opponent winning_lines
            # cover the full game (opening included) so preferred_plays still
            # capture complete winning lines from the very first move.
            if _late_boards:
                specialist_db.record_game(
                    _late_boards, _res, learner_moves_notation, "gen",
                    learner_color=learner_color,
                )
            # v4: also record opponent's move sequence as a winning_line when
            # the opponent wins — the AI can recognise those patterns if it
            # ever occupies the same positions on the board.
            if _res == "L" and opponent_moves_notation:
                specialist_db.record_game(
...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"[s_gen_v3] Device: {device}")
    ...
    # affects this v2c process; GUI / inference callers keep default levels.
    ...
    if getattr(args, "no_sentinel", False):
        print("[s_gen_v3] Sentinel disabled via --no-sentinel")
    else:
        ...
        if Path(sent_path).exists():
            ...
            if sentinel and sentinel.is_loaded():
                print(f"[s_gen_v3] Sentinel loaded: {sent_path}")
            else:
                sentinel = None
        if sentinel is None:
            print("[s_gen_v3] Sentinel unavailable — sentinel reward = 0")
    ...
    if malom_path and Path(malom_path).exists():
        try:
            ...
            if db.is_available():
                print(f"[s_gen_v3] Malom DB loaded (lookahead termination only): {malom_path}")
            else:
                db = None
        except Exception as e:
            print(f"[s_gen_v3] Malom DB failed ({e})")
    if db is None:
        print("[s_gen_v3] Malom DB unavailable — lookahead uses no endgame early-exit")

    value_net = None
    if getattr(args, "no_value_net", False):
        print("[s_gen_v3] Value net disabled via --no-value-net")
    else:
        ...
        if vn_path and Path(vn_path).exists():
            try:
                ...
                print(f"[s_gen_v3] Value net loaded: {vn_path}")
            except Exception as e:
                print(f"[s_gen_v3] Value net load failed ({e}) — VN features will be 0")
        else:
            print("[s_gen_v3] No value net — VN features will be 0")

    gap_net = None
    if getattr(args, "no_gap_net", False):
        print("[s_gen_v3] Gap net disabled via --no-gap-net")
    else:
        ...
        if gap_path and Path(gap_path).exists():
            try:
                ...
                print(f"[s_gen_v3] Gap net loaded: {gap_path}")
            except Exception as e:
                print(f"[s_gen_v3] Gap net load failed ({e}) — gap features will be 0.5")
        else:
            print("[s_gen_v3] No gap net — gap features will be 0.5")
        ...
        try:
            ...
            if Path(_pref_path).exists():
                ...
                if human_pref_adv is not None:
                    print(f"[s_gen_v3] HumanPrefAdvisor loaded: {_pref_path}")
        except Exception as e:
            print(f"[s_gen_v3] HumanPrefAdvisor load failed ({e}) — humanlike opponent disabled")
    ...
    if hdb_path.exists():
        try:
            ...
            print(f"[s_gen_v3] HumanDB loaded: {human_db.game_count} games "
                  f"({human_db.entry_count} positions)")
        except Exception as e:
            print(f"[s_gen_v3] HumanDB load failed ({e}) — human_freq features will be 0")
    else:
        print("[s_gen_v3] No HumanDB — human_freq features will be 0")
    ...
    print(f"[s_gen_v3] LookaheadAdvisor: 12-ply width, {args.sim_ply_depth}-ply sim, 5 signals (h+learner_sent+opp_sent+vn+gap)")

    # ── SpecialistDB ─────────────────────────────────────────────────────────
    specialist_db = SpecialistDB(Path(args.specialist_db))
    print(f"[s_gen_v3] SpecialistDB: {specialist_db.stats()}")
    ...
    if resume_path is None:
        print("[s_gen_v3] No checkpoint found — starting from scratch")
        _resume_ckpt: dict = {}
    else:
        print(f"[s_gen_v3] Resuming from ({source_tag}): {resume_path}  best_wr_at_diff={best_win_rate_at_diff:.3f}")
        try:
            _resume_ckpt = torch.load(str(resume_path), map_location="cpu", weights_only=False)
        except Exception:
            _resume_ckpt = {}
    print(f"[s_gen_v3] feat_dim={MOVE_FEAT_DIM_WITH_LOOKAHEAD}, starting game={start_game}, diff={difficulty}")
    ...
    print("[s_gen_v3] LookaheadAdvisor: frozen-model driven learner-side (Option C)")
    ...
    print(f"[s_gen_v3] Args saved → {_args_file.name}")
    ...
    if _resume_ckpt.get("optimizer_state") is not None:
        try:
            ...
            print("[s_gen_v3] Optimizer state restored from checkpoint")
        except Exception as e:
            print(f"[s_gen_v3] Optimizer restore failed ({e}) — starting fresh Adam state")
    ...
    for _rng_key, _restore in (
        ...
        if _saved is not None:
            try:
                ...
                print(f"[s_gen_v3] {_rng_key} restore failed ({e})")
    ...
    if _saved_cuda_rng is not None and torch.cuda.is_available():
        try:
            ...
            print(f"[s_gen_v3] rng_torch_cuda restore failed ({e})")
    ...
    print(f"[s_gen_v3] Starting at game {game_count}, difficulty {difficulty}")
    print(f"[s_gen_v3] Self-play ratio {args.self_play_ratio:.0%}, "
    ...
    if not args.no_s1a_warmstart and start_game == 0 and not getattr(args, "no_imitation_mix", False):
        print(f"[s_gen_v3] Running s1a warm-start (pre-RL imitation) from {args.s1a_data}")
    ...
    if _imitation_path.exists():
        try:
            ...
            print(f"[s_gen_v3] Imitation data loaded: {len(_imitation_data['feat_matrices'])} positions for mixing")
        except Exception as e:
            print(f"[s_gen_v3] Imitation data load failed ({e}) — imitation mixing disabled")
    ...
    if args.batch_games > 1:
        ...
        print(f"[s_gen_v3] Encoder warmed for {args.batch_games}-game parallel batches")
    ...
    _last_window_diff = None
    _sdb_window: "_DiffWindowDB | None" = None
    while game_count < segment_stop:
        batch_count += 1
        # v3: sim_ply_depth is fixed (not scaled with difficulty) so training
        # and inference see identically-quality lookahead features.
        # Rebuild the diff-windowed DB proxy whenever difficulty changes.
        if _last_window_diff != difficulty:
            if specialist_db is not None:
                _win_min = max(1, difficulty - args.sdb_diff_window + 1)
                _sdb_window = _DiffWindowDB(specialist_db, _win_min, difficulty)
            _last_window_diff = difficulty
        ...
        if advance_cooldown_batches > 0:
            ...
            if advance_cooldown_batches == 0:
                ...
                print(f"[s_gen_v3] Advance cooldown expired — accumulating fresh samples at diff {difficulty}")
        ...
        if hot_explore_remaining > 0:
            ...
            if hot_explore_remaining <= 0:
                print(f"[s_gen_v3] Hot-explore phase complete at game {game_count} — armed for full recovery if still failing")
        ...
        if games_since_target_update >= args.update_target_every:
            ...
            print(f"[s_gen_v3] Frozen model updated at game {game_count}")
        ...
        if recovery_grace > 0:
            ...
            if 0 < recovery_grace and recovery_grace % 25 < args.batch_games:
                ...
                print(f"[s_gen_v3] Recovery in progress ({_current_recovery_state}): "
                    ...
                    if best_ckpt.exists():
                        ...
                        if _ckpt_r.get("optimizer_state") is not None:
                            try:
                                ...
                                print(f"[s_gen_v3] Resurrection: optimizer state restore failed ({_oe}) — fresh Adam")
                        frozen_opp.refresh(model)
                        win_history.clear()
                        ...
                        print(f"[s_gen_v3] Post-grace resurrection: wr={_post_wr:.3f} vs "
                              f"baseline={recovery_baseline_win_rate:.3f} — reloaded best{difficulty}.pt "
                              f"(grace=50)")
                    else:
                        print(f"[s_gen_v3] Post-grace: wr={_post_wr:.3f} vs "
                              f"baseline={recovery_baseline_win_rate:.3f} — no checkpoint to resurrect")
                        _current_recovery_state = ""
                else:
                    print(f"[s_gen_v3] Recovery grace expired — draw penalty restored "
        ...
        for cfg, opponent, result in batch_results:
            ...
            if result.outcome == WIN_REWARD and not _primary_is_infra:
            ...
            elif result.outcome == DRAW_REP_PENALTY and not _primary_is_infra:
                # v2c: train on repetition-draw trajectories so the -0.6 signal propagates.
                ep_steps.extend(result.trajectory)
                ...
                if confirmed and result.trajectory:
                    ep_steps.extend(result.trajectory)
                if (confirm_result.outcome in (WIN_REWARD, DRAW_SHORT, DRAW_REP_PENALTY)
                        and not _confirm_is_infra):
                ...
                if game_count % 10 == 0:
                    print(f"[s_gen_v3] {game_count:6d}  r{game_retry_ply:2d} {learner_color} |          | {_coc} ply={confirm_result.ply:3d} | (from ply {game_retry_ply}) {'[learn]' if confirmed else '[skip]'}")
            ...
            if game_count % 10 == 0:
                ...
                print(f"[s_gen_v3] {game_count:6d} {_gt:4s} {learner_color} | {_dif} | {_oc} ply={result.ply:3d} | hwr={hwr:.3f} hdr={hdr:.3f} awr={_awr:.3f} | temp={temperature:.2f} lr={opt.param_groups[0]['lr']:.5f}")
            ...
            if (not args.minimal_rollouts
                ...
                if retry_result.trajectory and not _retry_is_infra:
                    ...
                    if retry_result.outcome in (WIN_REWARD, DRAW_SHORT, DRAW_REP_PENALTY):
                ...
                if game_count % 10 == 0:
                    print(f"[s_gen_v3] {game_count:6d} retry {learner_color} |          | {_roc} ply={retry_result.ply:3d} | (from ply {game_retry_ply})")
            ...
            for branch_ply, branch_board, bucket in ordered_candidates:
                ...
                if branch_result.trajectory and not _branch_is_infra:
                    ...
                    if branch_result.outcome in (WIN_REWARD, DRAW_SHORT, DRAW_REP_PENALTY):
                    ...
                    if game_count % 10 == 0:
                        ...
                        print(f"[s_gen_v3] {game_count:6d}  +b  {learner_color} | {bucket:7s} | {_boc} ply={branch_result.ply:3d} | (from ply {branch_ply})")
            ...
            if game_count - _last_log_game >= args.log_every and diag_buffer:
                ...
                if _in_stage2_grace:
                    ...
                    if _is_improving:
                        print(f"[s_gen_v3] Recovery skipped: AI is improving (score {_first_score:.3f} → {_second_score:.3f})")
                        if hot_explore_triggered and hot_explore_remaining <= 0:
                            hot_explore_triggered = False
                            _current_recovery_state = ""
                            print(f"[s_gen_v3] Hot-explore recovery successful — flags cleared")
                        ...
                        if best_ckpt.exists():
                            ...
                            try:
                                ...
                                try:
                                    model.load_state_dict(pol_state, strict=False)
                                    _loaded_recovery = True
                                    print(f"[s_gen_v3] Recovery: value_mlp shape mismatch — policy weights loaded, value head kept")
                                except RuntimeError:
                                    print(f"[s_gen_v3] Recovery: checkpoint shape incompatible — keeping current weights")
                            if _loaded_recovery:
                                model.to(device)
                                opt = torch.optim.Adam(model.parameters(), lr=args.lr)
                                if ckpt_r.get("optimizer_state") is not None:
                                    try:
                                        ...
                                        print(f"[s_gen_v3] Recovery Stage 2: optimizer state restore failed ({_oe}) — fresh Adam")
                                frozen_opp.refresh(model)
                                win_history.clear()
                                win_history_heuristic.clear()
                                temp_boost    = 0.0
                                ...
                                print(f"[s_gen_v3] Recovery Stage 2: reloaded best{difficulty}.pt "
                                      f"(W={win_rate:.2f} L={loss_rate:.2f})")
                                print(f"[s_gen_v3] Recovery grace: draw penalty suppressed for 100 games")
                            else:
                                _current_recovery_state = ""
                        else:
                            _current_recovery_state = ""
                            print(f"[s_gen_v3] Recovery Stage 2: no best{difficulty}.pt exists — cannot restore "
                                  f"(W={win_rate:.2f} L={loss_rate:.2f})")
                    elif not _is_degrading:
                        # Score is low but stable — don't fire Stage 1 on a plateau
                        print(f"[s_gen_v3] Recovery skipped: score low but stable ({_first_score:.3f} → {_second_score:.3f})")
                    else:
                        ...
                        if args.hot_explore_games > 0:
                            ...
                            print(f"[s_gen_v3] Recovery Stage 1: hot-explore for ~{args.hot_explore_games} iters "
                            ...
                            if best_ckpt.exists():
                                ...
                                if _loaded_recovery:
                                    ...
                                    _log_event("recovery_stage2",
                                               wr=round(win_rate, 4), loss=round(loss_rate, 4),
                                               checkpoint=str(best_ckpt))
                                    print(f"[s_gen_v3] Recovery: reloaded best{difficulty}.pt "
                                          f"(W={win_rate:.2f} L={loss_rate:.2f})")
                                    print(f"[s_gen_v3] Recovery grace: draw penalty suppressed for 100 games")
                                else:
                                    _current_recovery_state = ""
                                ...
                                _current_recovery_state = ""
                                print(f"[s_gen_v3] Recovery: no best{difficulty}.pt exists — cannot restore "
                                      f"(W={win_rate:.2f} L={loss_rate:.2f})")
                ...
                if last_main:
                    ...
                    print(
                        f"[s_gen_v3] game {game_count:6d} | diff {difficulty} | "
                ...
                if _should_save_best_checkpoint(win_rate, best_win_rate_at_diff, len(win_history_heuristic)):
                    ...
                    print(f"[s_gen_v3]  → best diff-{difficulty} win rate: {best_win_rate_at_diff:.3f}")
            ...
            if games_at_level >= 20:
                ...
                if _adv is not None and game_count - _last_advance_print_game >= 50:
                    ...
                    print(f"[s_gen_v3] advance-check @ diff {difficulty}: {_adv.reason}")
                ...
                if _adv is not None and _adv.should_advance:
                    ...
                    if _shadow_draw_rate >= _shadow_draw_cap:
                        print(f"[s_gen_v3] advance-check @ diff {difficulty}: "
                              f"[shadow] draw cap would have blocked "
                              f"(draw_rate={_shadow_draw_rate:.3f} >= {_shadow_draw_cap})")
            # v3 diff-max gate: require DIFF_MAX_COMPLETION_TARGET score at the
            # final difficulty level before declaring training done.
            if (_adv is not None and _adv.should_advance
                    and difficulty >= args.diff_max
                    and _adv.score_pct < DIFF_MAX_COMPLETION_TARGET):
                import dataclasses as _dc
                _adv = _dc.replace(
                    _adv,
                    should_advance=False,
                    reason=(f"diff-max gate: score={_adv.score_pct:.3f} "
                            f"< {DIFF_MAX_COMPLETION_TARGET} (keep training)"),
                )
            ...
            if _adv is not None and _adv.should_advance:
                if difficulty >= args.diff_max:
                    print(f"[s_gen_v3] *** DONE at diff {difficulty}: {_adv.reason} ***")
                    _advance_done = True
                    break
                else:
                    ...
                    level_heuristic_history.clear()
                    games_at_level = 0
                    print(f"[s_gen_v3] *** Advanced to diff {difficulty} (was diff {prev_diff}: "
                ...
                if not prev_best.exists():
                    ...
                    print(f"[s_gen_v3] Saved best{prev_diff}.pt at advancement (wr={wr:.3f})")
                ...
                print(f"[s_gen_v3] Continuing with in-memory model for diff {difficulty} (no checkpoint reload)")
                ...
                print(f"[s_gen_v3] Advancement boosts: temp+{temp_boost:.3f} "
    ...
    torch.save(ckpt, out_dir / "latest.pt")
    print(f"\n[s_gen_v3] Done. Games: {game_count}  Best win rate: {best_win_rate:.3f}")
...
def main() -> None:
    ...
    p.add_argument("--out-dir",   default=str(_ROOT / "learned_ai" / "checkpoints" / "scaffolded" / "s_gen_v4"))
    ...
    p.add_argument("--specialist-db", default=str(_ROOT / "data" / "specialist_db_v4.sqlite"),
                   help="Path to SpecialistDB SQLite file (v4 default: specialist_db_v4.sqlite)")
    p.add_argument("--sdb-diff-window", type=int, default=5,
                   help="Number of most-recently-played difficulty levels visible to the AI "
                        "when querying SpecialistDB during training (default 5). "
                        "The window is [difficulty - N + 1, difficulty].")
    ...
    p.add_argument("--sim-ply-depth",       type=int,   default=12,
                   help="LookaheadAdvisor simulation depth during training (v3 default: 12). "
                        "Fixed throughout training (not scaled with difficulty) to match the "
                        "inference default of 12, eliminating the training/inference feature mismatch.")
    ...
    p.add_argument("--no-sentinel",          action="store_true",
                   help="Disable sentinel even if the checkpoint exists")
    p.add_argument("--no-value-net",         action="store_true",
                   help="Disable value net even if the file exists")
    p.add_argument("--no-gap-net",           action="store_true",
                   help="Disable gap net even if the file exists")
    p.add_argument("--no-imitation-mix",     action="store_true",
                   help="Disable online imitation mini-steps during RL")
    p.add_argument("--heuristic-node-budget", type=int, default=None,
