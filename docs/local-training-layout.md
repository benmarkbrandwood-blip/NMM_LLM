...
| Retained-v3/v4 cross-corpus read-only view | `tools/serve_retained_phase_process_generalization.py` accepts the completed phase root through `--output-root` and the completed reused-development root through optional `--development-output-root`. It identity-checks the route/runtime/protocol basis, independently reaggregates both existing ledgers, and displays start-clustered survival, score precision and conservative fixed-width planning. It creates no game and grants no authority. The tracked synthesis is `docs/evidence/sanmill-retained-v3-v4-cross-corpus-synthesis-2026-08-14.md`. |
