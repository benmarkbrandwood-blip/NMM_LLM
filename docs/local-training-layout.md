...
| No-refresh retained-v4 attempt-003 SpecialistDB snapshot | Ignored closed snapshot at `data/specialist_db.no_refresh_retained_v4.attempt_003.template.sqlite`; copied byte-for-byte from the still-empty attempt-002 template, not from its failed runtime DB. It must remain 45,056 bytes with SHA-256 `5a5d8eb1df4184b1ed3581258ab2490f6b1320c7f9fd8a5322affeaf2cad540d`, `quick_check=ok`, `malom_label_version=sector-corrected-v1`, zero positions, winning lines and preferred plays, and no WAL, SHM or journal sidecar. Copy it once to the attempt-003 writable path only after the final source is published. |
