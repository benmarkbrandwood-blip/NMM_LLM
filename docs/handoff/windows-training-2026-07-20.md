...
The Web product now resolves Malom from environment, the ignored local path
registry, legacy shared settings, and Sentinel configuration in that order.
It validates every configured candidate against the tracked
`sector-corrected-v1` manifest, exact 512-file inventory, and component sizes,
then opens the first complete read-only oracle. On this host the local registry
passes at manifest identity `f4c52b00...`; the tracked WSL-style shared path is
reported as rejected. The Tools page now stores the Malom value only in the
ignored local registry.

Every adversarial AI route converges on one final `A_pos` choke before the game
engine applies a move. Classical difficulty 9/10, specialist, generalist, and
AI-versus-AI sources are covered. Lower default difficulties remain unfiltered
to preserve the difficulty ladder. Learned routes fall back to the already
computed classic move if Malom is unavailable; classic play remains available
but is explicitly logged and reported as unfiltered. This is position-only,
not history-aware `A_allow`.

Two real classical W-to-D placement/movement events were reproduced with the
exact frozen `main` runtime and blocked by live Malom. The focused run passed
27 tests; the mandatory Malom group passed 103 tests and 498 subtests. No game,
...
superseded before authorization so readiness evidence could be recorded
without invalidating itself. The published 156-game/two-hour machine plan is
