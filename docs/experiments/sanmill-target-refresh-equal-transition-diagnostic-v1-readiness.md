...
The current preparation stage may therefore create only three fresh prefix
plans, three isolated template-derived SpecialistDB files, and three
read-only preflight reports. A later arm-preparation stage must audit the
completed fork, clone the closed prefix database separately for both arms,
bind their exact identities, and generate six new plans and preflights.
