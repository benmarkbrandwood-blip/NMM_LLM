...
- 2026-08-15 — Codex 4th-pass review of `4e4a724` (BLOCK): three items addressed —
    - P1 artifact-lifecycle conflict: `--out` default was `data/gap_net_v3_stage_e_candidate.npz` but plan §12/§13/§16 artifacts table specifies `data/gap_net_v3_candidate.npz` as the Stage E output.  Fix: reverted default to `data/gap_net_v3_candidate.npz` (plan-owned name).  Stage E scope is captured in provenance.model label (`gap_net_v3_stage_e_candidate` / `gap_net_v3_stage_e_candidate_ineligible`), not in the filename.  The comment on the argparse `--out` argument updated to reflect this.
    - P2 frozen-registry composition not exercised end-to-end in main(): added `test_main_integration_stage_f_eligible_with_frozen_registry`.  This test writes a temp frozen registry, monkeypatches `_STAGE_E_THRESHOLDS_REGISTRY` + stubs `_report_gate` (returning PASS; gate math unit-tested elsewhere), runs `trainer.main()` with matching `--gate-thresholds-frozen-id` and default threshold CLI values, then asserts `gate_thresholds_frozen=True`, `gate_thresholds_frozen_id` / `sha256` / `entry` correct, `stage_f_eligible=True`, `promotion_eligible=False`, and model label `gap_net_v3_stage_e_candidate`.
    - P2 evidence count: Codex 4e4a724 recorded 221 passed + 9 skipped (fixture-gated skips); this commit adds the frozen-registry integration test and checklist update.
